...
furn('过道微波高柜','appliance',592,413,63,70,2.3)
furn('过道食品储物柜','cabinet',592,483,63,73,2.3)
furn('衣帽区洗烘叠放柜（待水电核对）','appliance',224,856,60,71,2.0)
...
D['change_notes']=['次卫墙线、门洞、门扇回转区全部沿用 PNG 原始位置；本版不再移动次卫门。','原家政柜列原位保留为厨房扩展：冰箱、微波高柜、食品储物柜从上到下集中布置。','原主通道右侧保持连续净宽约1.10m；标记3的转入位置不被家具占用。','厨房内释放原冰箱位，改为连续备餐台；厨房与过道共同承担备餐、冰箱、微波和食品收纳。','洗烘叠放暂放在衣帽区靠主卫一侧的柜位，属于水电可行性待核的可选项，不把它塞进次卫门前。']
...
for f in D['furniture']:
    ...
    elif f['name']=='过道微波高柜': base.append(text(x+w/2,y+h/2+6,'微波',13,weight=600))
    elif f['name']=='过道食品储物柜': base.append(text(x+w/2,y+h/2+6,'储物',13,weight=600))
    elif '洗烘叠放柜' in f['name']:
        base.append(f'<circle cx="{x+w/2}" cy="{y+h/2}" r="18" fill="#edf3ef" stroke="#93aaa0" stroke-width="1.5"/>')
        base.append(text(x+w/2,y+26,'洗烘',13,weight=600)); base.append(text(x+w/2,y+43,'叠放',13,weight=600))
...
base.extend([line([(440,446),(572,446)],'#63a99e',3),text(510,387,'独立淋浴',17),text(510,411,'玻璃分隔',13),text(672,305,'冰箱 / 微波 / 储物（原柜列）',11),text(705,596,'主通道保持畅通',13)])
...
for n,x,y in [(1,678,304),(2,650,420),(3,694,589),(4,1149,1218)]:
...
main+=text(200,83,'方案 A · 原动线保留版',31,anchor='start',weight=600)
...
main+=box(374,1373,24,14,'#bda47c','none')+text(408,1386,'厨房扩展家具',18,anchor='start')
...
detail+=dim(592,305,655,305,'原柜列约 0.62 m')
detail+=dim(410,160,410,305,'厨房净深约 1.50 m')
detail+=text(695,653,'原门位、柜列和主通道均保持；只重组家具功能',13)
detail+='</svg>'
...
assert 592 + 63 <= 655 and 556 < 561, 'Kitchen extension cabinet row must remain on the original side and finish before the bathroom door swing'
base_reference=json.loads((BASE/'plan_geometry.json').read_text())
report=dict(bath_width_m=round((580-428)*PX,2),bath_width_change_m=0.0,hall_clearance_m=round((767-655)*PX,2),bath_door_width_m=round(76*PX,2),washer_dryer_footprint_m=[round(60*PX,2),round(71*PX,2)],kitchen_width_m=round(364*PX,2),kitchen_depth_m=round(153*PX,2),kitchen_opening_m=round(175*PX,2),bedrooms=3,bathrooms=2,preserved_exterior=D['outer']==base_reference['outer'],preserved_walls=D['walls']==base_reference['walls'],preserved_bathroom_door=any(d['name']=='次卫门' and d['hinge']==[580,637] for d in D['doors']),status='Route-preserved concept; original wall and door geometry checked')
