...
> **RC1 + RC1.5 corrections applied** (owner reviews):
...
> 4. **RC1.5 (latest owner correction)**: cloakroom N/W/E walls are RETAINED, not demolished — "merged into secondary master" means the south side (Y4500-b segment) is open to the bedroom. Bath = existing shell + NEW south partition/frosted-glass + interior wet/dry glass. Zero demolition.
> 5. Bath entry verdicts inverted by truth: **S = FEASIBLE-RECOMMENDED** (door in new south partition, no existing wall touched); **W = CONSTRAINED** (needs opening in retained white-class CLK-W → owner exception + structural check); **N = fallback** (reuse existing cloakroom door, corridor-facing).
> 6. Conflict set now 1 wall: `Y4500-b-CLKSEG` (white-class yet opened — flagged for review, consistent with the merge).
...
Secondary master bath = cloakroom shell + NEW south partition/frosted glass + interior wet/dry glass. **No existing wall is demolished or cut for the recommended scheme.**
...
## 6. Secondary master bath — W vs S (RC1.5 verdict)

| Option | Verdict |
|---|---|
| **S** (door in NEW south partition → bedroom) | **FEASIBLE — RECOMMENDED**: zero demolition, uses the open south edge; door axis faces bed zone — mitigate via west-end placement or frosted glass |
| **W** (door cut in retained CLK-W → foyer) | **CONSTRAINED**: private vestibule + door not facing bed, but requires new opening in white-class retained wall → needs owner exception + structural check |
...
G1 source ✓ · G2 shell kept / S feasible / W constrained ✓ · G3 cabinets ✓ · G4 glazing ✓ · G5 beds ✓ · G6 3-piece ✓ · G7 balcony route ✓ · G8 living core ✓ · G9 ramp PROVISIONAL ✓ · G10 delta conflict ✓ · G10b balcony OPEN/PROPOSED ✓ · G11 study ✓ · G12 wet/dry ✓ · G13 KEEP ✓ · G14 program ✓
...
4. Residual stub x10800–11400 on site? (Option S door clearance.) Cloakroom N door leaf status.
5. Option W requires owner exception to open retained white-class CLK-W — decide S vs W.
