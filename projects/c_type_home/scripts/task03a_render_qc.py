...
def draw_walls(ax, mode='current'):
    for w in M['walls']:
        ...
        if w['disposition'] == 'EXISTING':
            c = WCOL['noopen'] if 'NO_DEMOLITION' in w['wall_class'] else WCOL['keep']
        else:
            c = WCOL['remove']
...
ax.text(st[0]-400, st[3]+150, '2 risers ~400mm TO_VERIFY', fontsize=7, color='#1a73e8')
ax.legend(handles=[Line2D([], [], color=WCOL['noopen'], lw=6, label='NO-OPENING(owner declared)'),
                   Line2D([], [], color=WCOL['keep'], lw=6, label='existing wall'),
...
ax.legend(handles=[Line2D([], [], color=WCOL['noopen'], lw=6, label='NO-OPENING keep'),
                   Line2D([], [], color=WCOL['keep'], lw=6, label='keep'),
...
for ax, opt in zip(axs, 'WS'):
    ...
    for w in M['walls']:
        ...
        if x2 > 7000 and x1 < 12500 and y2 > 3500 and y1 < 7500:
            c = WCOL['noopen'] if 'NO_DEMOLITION' in w['wall_class'] else WCOL['keep']
            if w['disposition'] != 'EXISTING': c = WCOL['remove']
            ax.add_patch(Rectangle((x1, y1), x2-x1, y2-y1, facecolor=c, zorder=2))
    if opt == 'W':
        ...
        ax.add_patch(Rectangle((8950, 4650), 100, 1800, facecolor=WCOL['new'], zorder=4))
        ax.add_patch(Rectangle((9050, 6300), 2250, 100, facecolor=WCOL['new'], zorder=4))
        ax.add_patch(Rectangle((9800, 4450), 1000, 200, facecolor='#d93025', alpha=.6, zorder=5))
        ax.text(7500, 5100, 'BLOCKED: needs new opening in\nOWNER_DECLARED_NO_OPENING wall\nY4500-b (white-fill 200mm+column)', fontsize=8, color='#d93025')
