...
def material(name,color,rough=.45,metal=.0, emission=None):
    ...
    return m
wood=material('白蜡木·木蜡油',(0.48,.30,.14),.42)
wood_light=material('白蜡木浅色',(0.68,.47,.25),.42)
wood_dark=material('白蜡木边框',(0.22,.11,.045),.42)
fabric=material('麻棉·暖灰',(0.38,.36,.32),.78)
fabric_light=material('麻棉·米灰',(0.58,.52,.44),.78)
wallmat=material('墙面·暖白',(0.78,.77,.72),.86)
ceilingmat=material('吊顶·纯白',(0.86,.86,.84),.92,0.0,emission=(0.72,0.72,0.70))
floorwood=material('地板·浅原木',(0.48,.33,.18),.54)
tile=material('瓷砖·暖灰',(0.56,.59,.56),.38)
tile_dark=material('瓷砖缝',(0.38,.42,.39),.7)
stone=material('台面·浅石材',(0.68,.65,.58),.32)
...
for entry in DATA['walls']: polygon_prism('Wall_'+entry['name'],entry['polygon'],0,2.62,wallmat)
...
# ceiling: a continuous flat white plane spanning the calibrated envelope.
# The polygon ceiling from V1 left dark background gaps at irregular corners; this slab keeps every interior camera enclosed.
ceiling=polygon_prism('平吊顶·白色',DATA['outer'],2.62,2.65,ceilingmat)
ceiling_cover=rect_obj('平吊顶·连续白色覆盖层',200,145,1285,1192,.24,ceilingmat,.006,2.50)
...
# lighting
world=bpy.context.scene.world or bpy.data.worlds.new('World'); bpy.context.scene.world=world; world.use_nodes=True; world.node_tree.nodes['Background'].inputs['Color'].default_value=(0.16,0.15,0.13,1); world.node_tree.nodes['Background'].inputs['Strength'].default_value=.55
# Soft daylight and low-energy warm pools keep the ash wood readable without washing out the room.
bpy.ops.object.light_add(type='SUN', location=(8,6,8)); sun=bpy.context.object; sun.name='全屋漫射日光'; sun.data.energy=.95; sun.data.angle=math.radians(30); sun.rotation_euler=(math.radians(28),math.radians(-18),math.radians(25))
def area(name,px,py,energy=450,size=3.0):
    bpy.ops.object.light_add(type='AREA',location=wp(px,py,2.25)); l=bpy.context.object; l.name=name; l.data.energy=energy; l.data.shape='DISK'; l.data.size=size; l.data.color=(1.0,.82,.62); return l
area('客餐厅暖光',980,1040,260,4); area('厨房暖光',600,230,190,3); area('主卧暖光',600,1000,220,3); area('老人房暖光',980,480,190,3); area('儿童房暖光',1300,950,190,3); area('主卫暖光',320,700,150,2); area('次卫暖光',500,470,120,2)
# camera
...
scene=bpy.context.scene; scene.render.engine='BLENDER_EEVEE'; scene.render.resolution_x=960; scene.render.resolution_y=720; scene.render.resolution_percentage=100; scene.render.image_settings.file_format='PNG'; scene.render.image_settings.color_mode='RGBA'; scene.render.film_transparent=False; scene.view_settings.look='AgX - Medium High Contrast'; scene.view_settings.exposure=-0.45; scene.render.image_settings.color_depth='8'; scene.render.resolution_percentage=100
# render overview with ceiling hidden, then interiors with ceiling visible
for key,c in cams.items():
    scene.camera=c; ceiling.hide_render=(key=='overview'); ceiling_cover.hide_render=(key=='overview')
    scene.render.filepath=str(OUT/f'{key}.png'); bpy.ops.render.render(write_still=True)
# restore and save
ceiling.hide_render=False
ceiling_cover.hide_render=False
