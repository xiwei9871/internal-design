...
class A1A2KnowledgeTests(unittest.TestCase):
    ...
    def test_a2_count_and_pattern_evidence(self):
        ...
        self.assertTrue(all((p["evidence_type"] in {"PROJECT_DERIVED", "RULE_DERIVED"} and p["project_basis"] and p["evidence"]) or len(set(p["precedent_ids"])) >= 2 for p in self.patterns))
        self.assertTrue(all((p["evidence_type"] in {"PROJECT_DERIVED", "RULE_DERIVED"} or len(p["evidence"]) == len(p["precedent_ids"])) and all(e["evidence"] and e["confidence"] for e in p["evidence"] ) for p in self.patterns))
        self.assertTrue(all(p["evidence_type"] in {"PROJECT_DERIVED", "RULE_DERIVED"} or p["evidence_confidence"] != "HIGH" or all(e.get("verification_status") == "VERIFIED_PRECEDENT" for e in p["evidence"]) for p in self.patterns))
        self.assertTrue(all(p["evidence_type"] == "RULE_DERIVED" for p in self.patterns if p["pattern_id"] in {"PAT-AGE-01", "PAT-BED-01", "PAT-BED-02", "PAT-BTH-01", "PAT-BTH-02", "PAT-KIT-02"}))
