...
# Scheme A2: bathroom comfort is retained while the corridor becomes a kitchen-extension/storage band.
removed_wall = next(w for w in D['walls'] if w['name']=='次卫家政隔墙')
D['walls'] = [w for w in D['walls'] if w['name']!='次卫家政隔墙']
newwalls = [
    dict(name='次卫东扩上段',polygon=rect(600,334,12,130),change='new'),
    dict(name='次卫东扩下段',polygon=rect(600,548,12,95),change='new'),
...
furn('过道冰箱','appliance',709,336,58,74,1.8)
furn('过道微波高柜','appliance',709,416,58,105,2.3)
furn('过道食品储物柜','cabinet',709,529,58,112,2.3)
furn('过道备餐台','lowcab',709,529,58,50,.85)
...
D['scheme']='A2 kitchen-extension layout'
D['change_notes']=[
    '次卫净宽由约1.49m扩大至约1.69m，东扩约0.20m；比 A1 少扩约0.15m，给过道厨房扩展留出条件。',
    '洗烘叠放放在次卫南端干区柜内；淋浴在北侧并设玻璃分隔。次卫改东侧外滑门。',
    '厨房内取消冰箱占位，保留连续橱柜与约1.72m通透移门；过道承担冰箱、微波炉和食品储物。',
    '过道东侧形成约0.9m净通行带：冰箱高柜、微波/储物高柜、食品储物柜，补回被取消的家政柜收纳。',
...
base.extend([line([(616,458),(616,634)],'#248e86',2),box(612,551,5,80,'#b7d7ce','#248e86',1),line([(440,446),(588,446)],'#63a99e',3),text(510,387,'独立淋浴',17),text(510,411,'玻璃分隔',13),text(660,285,'通透移门',14),text(662,480,'约0.9m净通道',14),text(738,326,'过道厨房扩展',13)])
...
routes=[[(1154,717),(1068,717),(1068,972),(975,1027),(944,1140)],
        [(1027,717),(665,717),(665,368),(670,280)],
        [(794,603),(665,603),(665,510),(638,510)]]
for pts in routes:
...
for n,x,y in [(1,678,430),(2,650,420),(3,650,589),(4,1149,1218)]:
...
main+=text(200,83,'方案 A · 厨房扩展优化版',31,anchor='start',weight=600)
main+=text(200,111,'3 卧室 · 餐客厅 · 1 厨房 · 2 卫生间 · A2',19,anchor='start',color='#74847a')
...
detail+=dim(612,365,709,365,'通道净宽约 0.9 m')
detail+=dim(410,160,410,305,'厨房净深约 1.50 m')
detail+=text(695,653,'冰箱＋微波＋食品储物集中在过道',13)
detail+='</svg>'
...
for sample in [(665,360),(665,450),(665,600),(604,502),(614,510),(671,300)]:
...
report=dict(bath_width_m=round((600-428)*PX,2),bath_width_change_m=round((600-580)*PX,2),
            hall_clearance_m=round((709-612)*PX,2),bath_door_width_m=round(84*PX,2),
